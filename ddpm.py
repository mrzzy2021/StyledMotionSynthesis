...
def Conv(kernel_size, in_channels, out_channels, dilation=1, stride=1, use_bias=True, kaiming=False, zeros=False):
    ...
    # pad_l = (dilation * (kernel_size - 1)) // 2
    # pad_r = dilation * (kernel_size - 1) - pad_l
    ...
    return conv
    # return nn.Sequential(*[pad((pad_l, pad_r)),conv])
...
class ResidualBlock2d(nn.Module):
    def __init__(self, temb_channels, in_channels, out_channels) -> None:
        ...
        # self.attn = LinearAttention(out_channels)
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, t, adj=None):
        ...
        y = x + t[:, :, None]
        # y, adj = self.gcn(y, adj)
        # y = self.dilated_conv(y) + cond
        y = self.conv1(y)
        y = swish(y)
        # gate = self.transformer(gate)
        y = self.norm2(y)
...
class Diffusion():
    ...
    def sample(self, batch_size, t, style, content):
        ...
        start = time.time()
        # seq = np.linspace(0, np.sqrt(0.8*self.num_diffusion_timesteps), 500)
        # seq = range(0, 1000, 200)
        # samples, fc = self.ddim_steps(x, seq, self.betas, style, content)
...
class Trainer():
    ...
    @staticmethod
    def convert_to_disc(
            raw):  # convert [B, (J - 1) * 3 + 4 (or 3, in 3dpos), T] to representations for the discriminator
        vraw = raw[..., 1:] - raw[...,
                              :-1]  # local velocity -> this differentiation is the same for positions & rotations
        ...
        while self.step < self.max_iter:
            ...
            if self.step % self.print_every == 0:
                ...
                torch.save({'model': self.model.state_dict(), 'optimiser': self.optim.state_dict(),
                            'ema': self.ema.state_dict(), 'step': self.step}, './ddpm.pt')
            self.step += 1
    ...
    def train_adv(self):
        ...
        while self.step < self.max_iter:
            ...
            real_loss = disc_loss(d_real, torch.ones_like(d_real))
            fake_loss = disc_loss(d_fake, torch.zeros_like(d_fake))
            l_disc = real_loss + fake_loss

            l_adv = disc_loss(d_fake, torch.ones_like(d_fake))
            l_feet = (foot_contact - fc).square().sum(dim=(1, 2)).mean(dim=0)
            ...
            loss = l_noise + l_feet + l_root + l_disc + 0.01 * l_velocity + 0.01 * l_acceleration
            loss.backward()
            self.optim.step()
...
class Generator:
    ...
    def generate(self, content_label, style_label, name):
        ...
        foot_contact = fc[0]
        save_bvh_from_network_output(output, output_path=f'./diff{name}.bvh')
        remove_fs(output, foot_contact, output_path=f'./fixed{name}.bvh')
